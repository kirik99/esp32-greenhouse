...
static esp_err_t _update_config_from_descriptor(const usb_config_desc_t *cfg_desc)
{
    ...
    do {
        already_next = false;
        ESP_LOGE(TAG, "Desc Type: 0x%02x, len: %d (class: 0x%02x, sub: 0x%02x, alt: %d)", next_desc->bDescriptorType, next_desc->bLength, context_class, context_subclass, context_intf_alt);
        ...
        case CS_INTERFACE_DESC:
            if (context_class == USB_CLASS_VIDEO && context_subclass == VIDEO_SUBCLASS_CONTROL) {
                ...
                if (context_intf_alt == 0) {
                    ...
                    const desc_header_t *header = (const desc_header_t *)next_desc;
                    ESP_LOGE(TAG, "Found VS descriptor subtype: 0x%02x, len: %d", header->bDescriptorSubtype, header->bLength);
                    {
                        char hex_buf[80] = {0};
                        for (int bi = 0; bi < header->bLength && bi < 24; bi++) {
                            sprintf(hex_buf + strlen(hex_buf), "%02X ", ((const uint8_t *)next_desc)[bi]);
                        }
                        ESP_LOGE(TAG, "  RAW: %s", hex_buf);
                    }
                    switch (header->bDescriptorSubtype) {
                    case VIDEO_CS_ITF_VS_INPUT_HEADER:
                        print_uvc_header_desc((const uint8_t *)next_desc, VIDEO_SUBCLASS_STREAMING);
                        break;
                    case VIDEO_CS_ITF_VS_STILL_IMAGE_FRAME: {
                        const uint8_t *raw = (const uint8_t *)next_desc;
                        uint8_t num_sizes = raw[4];
                        ESP_LOGE(TAG, "  >>> [VS_STILL_IMAGE_FRAME] endpoint: 0x%02X, num_sizes: %d", raw[3], num_sizes);
                        for (int si = 0; si < num_sizes && (5 + si * 4 + 3) < raw[0]; si++) {
                            uint16_t sw = raw[5 + si * 4] | (raw[5 + si * 4 + 1] << 8);
                            uint16_t sh = raw[5 + si * 4 + 2] | (raw[5 + si * 4 + 3] << 8);
                            ESP_LOGE(TAG, "      Still size [%d]: %u x %u", si + 1, sw, sh);
                        }
                        break;
                    }
                    case VIDEO_CS_ITF_VS_FORMAT_MJPEG:
                        parse_vs_format_mjpeg_desc((const uint8_t *)next_desc, &format_idx, &frame_num, &format);
                        ESP_LOGE(TAG, "  >>> [VS_FORMAT_MJPEG] format_idx: %d, frame_num: %d", format_idx, frame_num);
                        if (uvc_dev) {
                    ...
                    case VIDEO_CS_ITF_VS_FRAME_MJPEG: {
                        ...
                        ESP_LOGE(TAG, "  >>> [VS_FRAME_MJPEG] idx: %d, %u x %u, dflt_interval: %" PRIu32, _frame_idx, _frame_width, _frame_heigh, dflt_interval);
                        if (interval_type) {
                    ...
                    case VIDEO_CS_ITF_VS_FORMAT_FRAME_BASED:
                        ...
                        ESP_LOGE(TAG, "  >>> [VS_FORMAT_UNCOMPRESSED] format_idx: %d, frame_num: %d", format_idx, frame_num);
                        if (uvc_dev) {
                            uvc_frame_size_t *frame_size = uvc_dev->frame_size;
                            frame_size = (uvc_frame_size_t *)heap_caps_realloc(frame_size, frame_num * sizeof(uvc_frame_size_t), MALLOC_CAP_DEFAULT);
                            UVC_CHECK(frame_size, "alloc uvc frame size failed", ESP_ERR_NO_MEM);
                    ...
                    case VIDEO_CS_ITF_VS_FRAME_FRAME_BASED: {
                        ...
                        ESP_LOGE(TAG, "  >>> [VS_FRAME_UNCOMPRESSED] idx: %d, %u x %u, dflt_interval: %" PRIu32, _frame_idx, _frame_width, _frame_heigh, dflt_interval);
                        if (interval_type) {
                            min_interval = pp_interval[0];
                            for (size_t i = 0; i < interval_type; i++) {
                                if (usb_dev->uvc_cfg.frame_interval == pp_interval[i]) {
...
IRAM_ATTR static void _uvc_process_payload(_uvc_stream_handle_t *strmh, size_t req_len, uint8_t *payload, size_t payload_len)
{
    ...
    if (!flag_zlp) {
        ...
        if (payload_len >= payload[0]
                ...
                && (!reassemble || (strmh->frame_format != UVC_FRAME_FORMAT_MJPEG) || ((payload[payload[0]] == 0xff) && (payload[payload[0] + 1] == 0xd8)))
#endif
           ) {
